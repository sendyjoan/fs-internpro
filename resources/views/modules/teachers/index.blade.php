                ...
                <div class="row w-100">
                    ...
                    <div class="col-md-4 text-right">
                        @if (auth()->user()->can('administrator-create'))
                            <a href="{{ route('teachers.create') }}" class="btn btn-sm btn-primary text-white">Create</a>
                        @endif
                        ...
                        @if (auth()->user()->can('administrator-export'))
                            <a href="{{ route('teachers.create') }}" class="btn btn-sm btn-info text-white">Export</a>
                        @endif
                    </div>
                </div>
            </div>
                                ...
                                @if (auth()->user()->can('administrator-edit') || auth()->user()->can('administrator-delete'))
                                    <td>
                                        @if (auth()->user()->can('administrator-list'))
                                            <a href="{{ route('teachers.show', $user->id) }}" class="btn btn-sm btn-primary detail text-info">
                                                <i class="fas fa-info-circle"></i>
                                            </a>
                                        @endif
                                        @if (auth()->user()->can('administrator-edit'))
                                            <a href="{{ route('teachers.edit', $user->id) }}" class="btn btn-sm btn-primary update text-warning">
                        ...
                        <div class="modal-body">
                            <form action="{{ route('teachers.create') }}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="row">
                                    ...
                                    <p>Unknow the template? <a href='{{ route('teachers.create') }}'>Click here to download!</a></p>
                                    </div>
                                </div>
                            </div>
