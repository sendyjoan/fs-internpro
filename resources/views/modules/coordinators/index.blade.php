                ...
                <div class="row w-100">
                    ...
                    <div class="col-md-4 text-right">
                        ...
                        @if (auth()->user()->can('administrator-export'))
                            <a href="{{ route('coordinators.create') }}" class="btn btn-sm btn-info text-white">Export</a>
                        @endif
                    </div>
                </div>
            </div>
                                ...
                                @if (auth()->user()->can('administrator-edit') || auth()->user()->can('administrator-delete'))
                                    <td>
                                        @if (auth()->user()->can('administrator-list'))
                                            <a href="{{ route('coordinators.show', $user->id) }}" class="btn btn-sm btn-primary detail text-info">
                                                <i class="fas fa-info-circle"></i>
                                            </a>
                                        @endif
                                        @if (auth()->user()->can('administrator-edit'))
                                            <a href="{{ route('coordinators.edit', $user->id) }}" class="btn btn-sm btn-primary update text-warning">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                        @endif
                                        @if (auth()->user()->can('administrator-delete'))
                                            <form action="{{ route('coordinators.destroy', $user->id) }}" method="POST" class="d-inline">
                                                @csrf
                                                @method('DELETE')
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form action="{{ route('coordinators.create') }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                <div class="row">
                                    <div class="col-md-12">
                                    ...
                                    <p>Unknow the template? <a href='{{ route('coordinators.create') }}'>Click here to download!</a></p>
                                    </div>
                                </div>
                            </div>
