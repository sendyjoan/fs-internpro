                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="form-group">
                                ...
                                <input type="text" value="{{ !empty(old('name')) ? old('name') : $class->name }}" class="form-control @error('name') is-invalid @enderror" id="name" name="name">
                                @error ('name')
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control @error('major_id') is-invalid @enderror" id="major_id" name="major_id">
                                    ...
                                    @foreach ($majors as $major)
                                        <option value="{{ $major->id }}" {{ $class->major_id == $major->id || old('major_id') == $major->id ? 'selected' : '' }}>{{ $major->name }}</option>
                                    @endforeach
                                </select>
                                @error ('major_id')
                                    <div class="invalid-feedback">{{ $message }}</div>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control @error('school_id') is-invalid @enderror" id="school_id" name="school_id">
                                    ...
                                    @foreach ($schools as $school)
                                        <option value="{{ $school->id }}" {{ $class->school_id == $school->id || old('school_id') == $school->id ? 'selected' : '' }}>{{ $school->name }}</option>
                                    @endforeach
                                </select>
                                @error ('school_id')
                                    <div class="invalid-feedback">{{ $message }}</div>
